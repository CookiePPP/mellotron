    ...
    def forward(self, ref, ref_mode=1):
        ...
        if np.random.uniform(0.0, 1.0) <= self.p_drop_tokens and self.training: # if drop_tokens
            if self.drop_tokens_mode == 'embedding':
                ...
                A = self.speaker_embedding(ref) # ref = speaker_ids
            elif self.drop_tokens_mode == 'zeros':
                A = torch.zeros(ref.shape[0],self.token_num).cuda()
            elif self.drop_tokens_mode == 'halfs':
                A = torch.ones(ref.shape[0],self.token_num).cuda() * 0.5
        
        else: # normal reference mode
            if ref_mode == 1: # get style_token from spectrogram
                ref = self.ref_encoder(ref) # (Batch, 1, Embedding)
                A = self.att(ref, token_embedding) # (Batch, token_num)
            elif ref_mode == 0:# get style_token from user input
                A = ref
            elif ref_mode == 2: # infer style_token from input_text and torchMoji
                #attHidden = torchMoji(ref) # ref=input_text
                A = self.map_lin(attHidden) # A = style_tokens
            elif ref_mode == 3: # training for mapping torchMoji attHidden to tokens
                A = self.map_lin(ref) # ref = torchMoji attention hidden, A = style_tokens
        
        # Apply Activation function
        if self.activation_fn == 0: A = torch.softmax(A, dim=-1)
        elif self.activation_fn == 1: A = A.sigmoid()
        elif self.activation_fn == 2: A = A.tanh()
        elif self.activation_fn == 3: A = A
